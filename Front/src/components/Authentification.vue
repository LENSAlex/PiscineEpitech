<template>
  <div class="container">
    <div class="row">
        ...
        <div class="col-md-4">
          <div class="card">
                <form @submit.prevent="SubmitUser" action="https://vuejs.org/" method="post" class="box">
                    <h1>Create User</h1>
                    <p class="text-muted"> Please enter a username and email</p> 
...
export default {
  ...
  methods : {
    ...
    getUser (email , username) {
      axios.get('http://localhost:4000/api/users?email='+email+'&username='+username).then(response => {
        this.$router.push({path : '/User' , props : {idUser : response.data.data.id}});
      })
    },
