<template>
  <div class="container">
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="user in users" :key="user.email">
          ...
          <th>
            <button
              v-on:click="updateUserDisplay(user.id)"
              :data-id="user.id"
    ...
    <form
      id="DisplayPutUser"
      @submit.prevent="UpdateUser(idUserPut)"
      method="post"
      v-bind:data-id="idUserPut"
      class="FormulairePutUser border border-primary"
      v-show="FormulaireStats"
      style="margin: 10px"
    >
      <p>
